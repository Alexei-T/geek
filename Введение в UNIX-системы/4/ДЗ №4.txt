ЗАДАНИЕ:
1)Написать скрипт, который удаляет из текстового файла пустые строки и заменяет маленькие символы на большие (воспользуйтесь tr или sed).
2)Создать скрипт, который создаст директории для нескольких годов (2010 — 2017), в них — поддиректории для месяцев (от 01 до 12),и в каждый из них запишет несколько файлов с
произвольными записями (например, 001.txt, содержащий текст«Файл 001», 002.txt с текстом Файл 002) и т. д.
* Создать файл crontab, который ежедневно регистрирует занятое каждым пользователем дисковое пространство в его домашней директории.
* Создать скрипт ownersort.sh, который в заданной папке копирует файлы в директории, названные по имени владельца каждого файла. Учтите, что файл должен принадлежать
соответствующему владельцу.

РЕШЕНИЕ:
1)
#!/bin/bash
echo -n "Введите (через пробел или табуляцию) полный путь до директории с файлом, имя файла:"
read var1 var2
if [[ -d $var1 && -f $var2 ]]; then
cd $var1
sed -i -e '/^$/d' -e 's/[a-z]/\U&/g' $var2
echo "В указанном файле пустые строки удалены, маленькие символы заменены на большие"
else
echo -n "Неверно указан путь либо имя файла"
fi
___________________________________________________________________________________________________________________________________________________________________________________
2)
#!/bin/bash
for i in {2010..2017}
do
for j in {01..12}
do
mkdir -p $i/$j && echo "file $j" > $i/$j/$j.txt
done
done
___________________________________________________________________________________________________________________________________________________________________________________
*
touch ~/dz3.sh
chmod +x ~/dz3.sh

#Содержимое dz3.sh 
#!/bin/bash
for i in  /home/*
do
du -hs $i > $i/$(echo $i |cut -d "/" -f3).txt
done
du -hs /home/* > /root/total.txt

#Скрипт будет в домашней директории пользователей создавать файл с информацией по размеру его домашней директории. Для рута будет создан файл с информацией по 
каждому пользователю.

echo "6 3 * * * root ~/dz3.sh" >> /etc/crontab
#Созданый скрипт будет запускаться каждый день в 03:06
_________________________________________________________________________________________________________________________________________________________________________________
*
#!/bin/bash
echo -n "Введите полный путь до директории с файлами для сортировки:"
read indir
echo -n "Введите полный путь новой директории, если такая директория уже существует, то она будет перезаписана:"
read outdir
if ! [ -d $outdir ]; then
mkdir $outdir
else
rm -rf $outdir/*
fi
for i in $(find $indir/* -printf '%u:%h:%p:%P\n') 
do
own=$(echo $i | cut -d ':' -f1)  
file=$(echo $i | cut -d ':' -f3)
outfile=$outdir/$own
[ -d $file ] && continue 
[ -d $outfile ] || mkdir -p $outfile
cp -fp $file $outfile/
chown $own $outfile
done

